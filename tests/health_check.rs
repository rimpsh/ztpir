...
static TRACING: Lazy<()> = Lazy::new(|| {
    ...
    if std::env::var("TEST_LOG").is_ok() {
        let macgyver = get_subscriber(subscriber_name, default_filter_level, std::io::stdout);
        init_subscriber(macgyver);
    } else {
        let macgyver = get_subscriber(subscriber_name, default_filter_level, std::io::sink);
        init_subscriber(macgyver);
    };
});
